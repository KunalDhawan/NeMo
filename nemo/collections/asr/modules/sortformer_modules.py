...
class SortformerModules(NeuralModule, Exportable):
    ...
    def __init__(
        ...
        spkcache_len: int = 188, # 188
        fifo_len: int = 188, # 188
        chunk_len: int = 152, # 376
        spkcache_refresh_rate: int = 50, # 0
        ...
        self.fifo_len_audio = int(fifo_len / 12.5 * 16000)
        self.fifo_len_feat = int(fifo_len / 12.5 * 100)
        self.spkcache_len_feat = int(spkcache_len / 12.5 * 100)
        self.spkcache_len_audio = int(spkcache_len / 12.5 * 16000)
        self.chunk_len = chunk_len
        self.chunk_len_audio = int(chunk_len / 12.5 * 16000)
    ...
    def init_streaming_state(
        ...
        if async_streaming:
            ...
            streaming_state.spkcache_audio = torch.zeros((batch_size, self.spkcache_len_audio), device = device)
            streaming_state.fifo_audio = torch.zeros((batch_size, self.fifo_len_audio), device = device)
            streaming_state.spkcache_audio_lengths = torch.zeros((batch_size,), dtype=torch.long, device = device)
            streaming_state.fifo_audio_lengths = torch.zeros((batch_size,), dtype=torch.long, device = device)
        else:
            ...
            streaming_state.spkcache_feat = torch.zeros((batch_size, 0, 80), device = device)
            streaming_state.fifo_feat = torch.zeros((batch_size, 0, 80), device = device)
            streaming_state.spkcache_audio = torch.zeros((batch_size, 0), device = device)
            streaming_state.fifo_audio = torch.zeros((batch_size, 0), device = device)
    ...
    def streaming_update_async_audio(
        self,
        streaming_state,
        chunk_audio,
        chunk_audio_lengths,
        ...
        batch_size, _ = chunk_audio.shape
        n_spk = preds.shape[2]
        max_spkcache_len_audio, max_fifo_len_audio, max_chunk_len_audio = streaming_state.spkcache_audio.shape[
            1], streaming_state.fifo_audio.shape[1], chunk_audio.shape[1] - lc - rc
        
        # max_spkcache_len = int(max_spkcache_len_audio / 16000 * 12.5)
        # max_fifo_len = int(max_fifo_len_audio / 16000 * 12.5)
        # max_chunk_len = int(max_chunk_len_audio / 16000 * 12.5)
        ...
        updated_fifo_audio = torch.zeros((batch_size, max_fifo_len_audio + max_chunk_len_audio), device=preds.device)

        updated_spkcache_audio = torch.zeros((batch_size, max_spkcache_len_audio), device=preds.device)
        ...
        for batch_index in range(batch_size):
            ...
            spkcache_len_audio = streaming_state.spkcache_audio_lengths[batch_index].item()
            fifo_len_audio = streaming_state.fifo_audio_lengths[batch_index].item()
            chunk_len_audio = chunk_audio_lengths[batch_index].item()
            ...
            updated_fifo_audio[batch_index, :fifo_len_audio] = streaming_state.fifo_audio[batch_index, :fifo_len_audio]


            # append chunk to fifo
            streaming_state.fifo_audio_lengths[batch_index] += chunk_len_audio
            updated_fifo_audio[batch_index, fifo_len_audio:fifo_len_audio + chunk_len_audio] = chunk_audio[batch_index, lc:lc + chunk_len_audio]

            if fifo_len_audio + chunk_len_audio > max_fifo_len_audio:
                if spkcache_len_audio == 0:
                    #update memroy only once to be query + separater
                    pop_out_len_audio = streaming_state.query_len_audio[batch_index]
                    updated_spkcache_audio[batch_index, :pop_out_len_audio] = updated_fifo_audio[batch_index, :pop_out_len_audio]
                    streaming_state.spkcache_audio[batch_index, :pop_out_len_audio] = updated_spkcache_audio[batch_index, :pop_out_len_audio]
                    streaming_state.spkcache_audio_lengths[batch_index] = pop_out_len_audio
                else:
                    pop_out_len_audio = fifo_len_audio + chunk_len_audio - max_fifo_len_audio
                # pop_out_len = int(pop_out_len_audio / 16000 * 12.5)
                
                streaming_state.fifo_audio_lengths[batch_index] -= pop_out_len_audio
                new_fifo_len_audio = streaming_state.fifo_audio_lengths[batch_index].item()
                updated_fifo_audio[batch_index, :new_fifo_len_audio] = updated_fifo_audio[
                    batch_index, pop_out_len_audio:pop_out_len_audio + new_fifo_len_audio
                ].clone()
                updated_fifo_audio[batch_index, new_fifo_len_audio:] = 0

        streaming_state.fifo_audio = updated_fifo_audio[:, :max_fifo_len_audio]
    ...
    def streaming_update_async(
        ...
        updated_spkcache = torch.zeros((batch_size, max_spkcache_len, emb_dim), device=preds.device)
        ...
        for batch_index in range(batch_size):
            ...
            if fifo_len + chunk_len > max_fifo_len:
                if spkcache_len == 0:
                    #update memroy only once to be query + separater
                    pop_out_len = streaming_state.query_pred_len[batch_index]
                    updated_spkcache[batch_index, :pop_out_len, :] = updated_fifo[batch_index, :pop_out_len, :]
                    streaming_state.spkcache[batch_index, :pop_out_len, :] = updated_spkcache[batch_index, :pop_out_len, :]
                    streaming_state.spkcache_lengths[batch_index] = pop_out_len
                else:
                    pop_out_len = fifo_len + chunk_len - max_fifo_len
    ...
    def streaming_update_audio(
        ...
        """
        update audio streaming state
        """
        spkcache_len_audio, fifo_len_audio, chunk_len_audio = streaming_state.spkcache_audio.shape[
            1], streaming_state.fifo_audio.shape[1], chunk_audio.shape[1]
        
        spkcache_len = int(spkcache_len_audio / 16000 * 12.5)
        fifo_len = int(fifo_len_audio / 16000 * 12.5)
        chunk_len = int(chunk_len_audio / 16000 * 12.5)

        streaming_state.fifo_preds = preds[:, spkcache_len:spkcache_len + fifo_len]
        chunk_preds = preds[:, spkcache_len + fifo_len :spkcache_len + fifo_len + chunk_len ]

        # append chunk to fifo
        streaming_state.fifo_audio = torch.cat([streaming_state.fifo_audio, chunk_audio], dim=1)
        streaming_state.fifo_preds = torch.cat([streaming_state.fifo_preds, chunk_preds], dim=1)

        if fifo_len_audio + chunk_len_audio > self.fifo_len_audio:
            if streaming_state.spkcache_audio.shape[1] == 0:
                #update memory only once to be query + separater
                streaming_state.spkcache_audio = streaming_state.fifo_audio[:,:self.spkcache_len_audio]
                pop_out_len_audio = self.spkcache_len_audio
            else:
                pop_out_len_audio = fifo_len_audio + chunk_len_audio - self.fifo_len_audio
            pop_out_len = int(pop_out_len_audio / 16000 * 12.5)
            streaming_state.fifo_audio = streaming_state.fifo_audio[:, pop_out_len_audio:]
            streaming_state.fifo_preds = streaming_state.fifo_preds[:, pop_out_len:]
        # print(streaming_state.spkcache_audio.shape)
        # print(streaming_state.fifo_audio.shape)
        return streaming_state, chunk_preds

    # def streaming_update_feat(
    #     self,
    #     streaming_state,
    #     chunk_feat,
    #     preds,
    ...
    #     update audio streaming state
    #     """
    #     spkcache_len_feat, fifo_len_feat, chunk_len_feat = streaming_state.spkcache_feat.shape[
    #         1], streaming_state.fifo_feat.shape[1], chunk_feat.shape[1]
        
    #     spkcache_len = int(spkcache_len_feat / 100 * 12.5)
    #     fifo_len = int(fifo_len_feat / 100 * 12.5)
    #     chunk_len = int(chunk_len_feat / 100 * 12.5)

    #     streaming_state.fifo_preds = preds[:, spkcache_len:spkcache_len + fifo_len]
    #     chunk_preds = preds[:, spkcache_len + fifo_len :spkcache_len + fifo_len + chunk_len ]

    #     # pop_out_len is the number of frames we will pop out from FIFO to update spkcache
    #     pop_out_len_feat = int(self.spkcache_refresh_rate / 12.5 * 100)
    #     pop_out_len = self.spkcache_refresh_rate

    #     # append chunk to fifo
    #     streaming_state.fifo_feat = torch.cat([streaming_state.fifo_feat, chunk_feat], dim=1)
    #     streaming_state.fifo_preds = torch.cat([streaming_state.fifo_preds, chunk_preds], dim=1)

    #     if fifo_len_feat + chunk_len_feat > self.fifo_len_feat:
    #         # extract pop_out_len first frames from FIFO queue
    #         pop_out_len_feat = min(pop_out_len_feat, fifo_len_feat + chunk_len_feat)
    #         pop_out_feat = streaming_state.fifo_feat[:, :pop_out_len_feat]
    #         pop_out_preds = streaming_state.fifo_preds[:, :pop_out_len]
    #         streaming_state.fifo_feat = streaming_state.fifo_feat[:, pop_out_len_feat:]
    #         streaming_state.fifo_preds = streaming_state.fifo_preds[:, pop_out_len:]

    #         # append pop_out_embs to spkcache
    #         if streaming_state.spkcache.shape[1] == 0:
    #             #update memory only once
    #             # streaming_state.spkcache = pop_out_embs[:,:50]
    #             import ipdb; ipdb.set_trace()
    #             streaming_state.spkcache_feat = pop_out_feat[:,:self.spkcache_len_feat]
    #         else:
    #             pass
    ...
    #             pass
    #         else:
    #             streaming_state.spkcache_preds = pop_out_preds[:,:self.spkcache_len]
    ...
    def streaming_update(
        ...
        if fifo_len + chunk_len > self.fifo_len:
            ...
            # if streaming_state.spkcache.shape[1] == 0:
            #     #update memory only once
            #     streaming_state.spkcache = pop_out_embs[:,:self.spkcache_len]
            # else:
            #     pass
            # if streaming_state.spkcache_preds is not None:  # if speaker cache has been already updated at least once
            #     # streaming_state.spkcache_preds = torch.cat([streaming_state.spkcache_preds, pop_out_preds], dim=1)
            ...
            if streaming_state.spkcache.shape[1] == 0:
                #update memory only once to be query + separater
                streaming_state.spkcache = streaming_state.fifo[:,:self.spkcache_len]
                pop_out_len = self.spkcache_len
            else:
                pop_out_len = fifo_len + chunk_len - self.fifo_len
            streaming_state.fifo = streaming_state.fifo[:, pop_out_len:]
            streaming_state.fifo_preds = streaming_state.fifo_preds[:, pop_out_len:]


            # print(streaming_state.spkcache.shape)
            # print(streaming_state.fifo.shape)
