...
class SortformerCLSEncLabelModel(ModelPT, ExportableEncDecModel, SpkDiarizationMixin):
    ...
    This model extends Sortformer by using CLS tokens from the encoder to obtain
    speaker embeddings, and supports multiple backends for computing speaker logits:
        * trff: Transformer + feedforward (original Sortformer approach)
        * dotp: Dot product between frame and speaker embeddings
        * isd: Individual Speaker Detection with per-speaker encoders
        * jsd: Joint Speaker Detection with time-speaker attention

    This model class expects config dict for:
        * preprocessor
        * FastConformer Encoder (with CLS tokens)
        * SortformerCLS Modules
        * Backend-specific encoder (transformer_encoder, isd_encoder, or jsd_encoder)
    """
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        if not hasattr(self.encoder, 'num_cls_tokens') or self.encoder.num_cls_tokens < self.sortformer_modules.n_spk:
            raise ValueError(
                "CLS-based encoder must be specified in the config and have at least num_spks CLS tokens"
            )
        ...
        if self.sortformer_modules.backend == "trff":
            ...
            self.transformer_encoder = SortformerCLSEncLabelModel.from_config_dict(self._cfg.transformer_encoder).to(self.device)
        else:
            self.transformer_encoder = None

        # Optional ISD backend modules
        if self.sortformer_modules.backend == "isd":
            if not hasattr(self._cfg, "isd_encoder"):
                raise ValueError("isd backend requires 'isd_encoder' config")
            self.isd_encoder =  SortformerCLSEncLabelModel.from_config_dict(self._cfg.isd_encoder).to(self.device)
        else:
            self.isd_encoder = None

        # Optional JSD (Joint Speaker Detection) backend modules
        if self.sortformer_modules.backend == "jsd":
            if not hasattr(self._cfg, "jsd_encoder"):
                raise ValueError("jsd backend requires 'jsd_encoder' config")
            self.jsd_encoder =  SortformerCLSEncLabelModel.from_config_dict(self._cfg.jsd_encoder).to(self.device)
        else:
    ...
    def backend_isd(self, emb_seq, emb_seq_length, spk_embs):
        ...
        combined = self.sortformer_modules.apply_fusion(emb_seq_expanded, spk_embs)
