...
class SpeakerKernelMixin:
    ...
    def _init_spk_kernel(self):
        ...
        for layer_idx in self.spk_kernel_layers:
            if kernel_class is not None:
                self.spk_kernels[str(layer_idx)] = kernel_class(
                    hidden_size, hidden_size, 
                    mask_original=self.spk_kernel_mask_original, 
                    residual=self.spk_kernel_residual
                )
                if self.add_bg_spk_kernel:
                    self.bg_spk_kernels[str(layer_idx)] = kernel_class(
                        hidden_size, hidden_size, 
                        mask_original=self.spk_kernel_mask_original, 
                        residual=self.spk_kernel_residual
                    )
    ...
    def _get_bg_spk_kernel_hook_pre_layer(self, layer_idx: str):
        ...
        def hook_fn(module, args, kwargs):
            if self.bg_spk_targets is None:
                return args, kwargs
    ...
    def _get_bg_spk_kernel_hook_post_layer(self, layer_idx: str):
        ...
        def hook_fn(module, input, output):
            ...
            if isinstance(output, tuple):
                ...
                x = self.bg_spk_kernels[layer_idx](x, self.bg_spk_targets)
                return (x, *cache)
            return self.bg_spk_kernels[layer_idx](output, self.bg_spk_targets)
