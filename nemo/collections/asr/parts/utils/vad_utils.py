...
def binarization_vectorized(sequence: torch.Tensor, per_args: Dict[str, float]) -> torch.Tensor:
    ...
    Args:
        ...
        per_args: Dict with keys onset, offset, pad_onset, pad_offset,
                  frame_length_in_sec.
    ...
    padded = torch.nn.functional.pad(above.unsqueeze(0).float(), (1, 1), value=0.0).squeeze(0)
...
def predlist_to_timestamps_fast_pp(
    batch_preds_list: List[torch.Tensor],
    audio_rttm_map_dict: Dict[str, Dict[str, Union[float, int]]],
    cfg_vad_params,
    unit_10ms_frame_count: int,
    ...
    """
    Fast version of predlist_to_timestamps that supports full postprocessing.

    Uses binarization_vectorized instead of the Python-loop binarization,
    and operates at native frame resolution (no repeat_interleave).
    ...
    Args:
        ...
        audio_rttm_map_dict (Dict[str, Dict[str, Union[float, int]]]):
            Dictionary mapping unique audio file names to their rttm file entries.
        cfg_vad_params:
            Configuration of VAD parameters (OmegaConf or dict).
        unit_10ms_frame_count (int):
            Number of 10ms frames in a unit (e.g. 8 for 80ms frames).
        bypass_postprocessing (bool, optional):
            If True, only binarization is applied. Defaults to False.
        precision (int, optional):
            Decimal places for rounding timestamps. Defaults to 2.

    Returns:
        total_speaker_timestamps: nested list [session][speaker][[start, end], ...]
    """
    total_speaker_timestamps = []
    pp_message = "Binarization" if bypass_postprocessing else "Post-processing"
    ...
    for sample_idx, (uniq_id, audio_rttm_values) in tqdm(
        ...
        for spk_id in range(speaker_assign_mat.shape[-1]):
            ...
            if speech_segments.numel() > 0:
                ...
                ts_seg_raw_list = ts_mat.tolist()
                ts_seg_list = [
                    [round(stt, precision), round(end, precision)]
                    for (stt, end) in ts_seg_raw_list
                ]
            else:
                ts_seg_list = []
