...
class ResidualAddAdapterStrategy(AbstractAdapterStrategy):
    ...
    def forward(self, input: torch.Tensor, adapter: torch.nn.Module, *, module: 'AdapterModuleMixin', additional_input={}):
        """
        ...
        Returns:
            The result tensor, after one of the active adapters has finished its forward passes.
        """
        out = self.compute_output(input, adapter, module=module, additional_input=additional_input)
    ...
    def compute_output(
        self, input: torch.Tensor, adapter: torch.nn.Module, *, module: 'AdapterModuleMixin', additional_input={}
    ) -> torch.Tensor:
        """
        Compute the output of a single adapter to some input.
        ...
        Returns:
            The result tensor, after one of the active adapters has finished its forward passes.
        """
        out = adapter(input, **additional_input)
        return out
