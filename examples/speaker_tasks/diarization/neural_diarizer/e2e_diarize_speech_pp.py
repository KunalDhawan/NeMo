...
Uses vectorized postprocessing (binarization_vectorized + filtering) for significantly faster
evaluation and Optuna optimization compared to the original e2e_diarize_speech.py.
...
python $BASEPATH/neural_diarizer/e2e_diarize_speech_pp.py \
...
from nemo.collections.asr.parts.utils.vad_utils import (
    ...
    predlist_to_timestamps_fast_pp,
)
...
class DiarizationConfig:
    ...
    optuna_onset_max: float = 0.8
    optuna_offset_min: float = 0.4
    optuna_offset_max: float = 0.8
    optuna_pad_onset_min: float = 0.0
    optuna_pad_onset_max: float = 0.5
    optuna_pad_offset_min: float = 0.0
    optuna_pad_offset_max: float = 0.3
    optuna_min_duration_on_min: float = 0.0
    optuna_min_duration_on_max: float = 0.75
    optuna_min_duration_off_min: float = 0.0
    optuna_min_duration_off_max: float = 0.75
    optuna_step: float = 0.01


def optuna_suggest_params(
    postprocessing_cfg: PostProcessingParams, trial: optuna.Trial, cfg: DiarizationConfig
) -> PostProcessingParams:
    """
    ...
    Args:
        ...
        cfg (DiarizationConfig): The configuration object containing Optuna search ranges.

    Returns:
        PostProcessingParams: The updated postprocessing configuration with suggested hyperparameters.
    """
    step = cfg.optuna_step
    postprocessing_cfg.onset = trial.suggest_float("onset", cfg.optuna_onset_min, cfg.optuna_onset_max, step=step)
    postprocessing_cfg.offset = trial.suggest_float("offset", cfg.optuna_offset_min, cfg.optuna_offset_max, step=step)
    postprocessing_cfg.pad_onset = trial.suggest_float("pad_onset", cfg.optuna_pad_onset_min, cfg.optuna_pad_onset_max, step=step)
    postprocessing_cfg.pad_offset = trial.suggest_float("pad_offset", cfg.optuna_pad_offset_min, cfg.optuna_pad_offset_max, step=step)
    postprocessing_cfg.min_duration_on = trial.suggest_float("min_duration_on", cfg.optuna_min_duration_on_min, cfg.optuna_min_duration_on_max, step=step)
    postprocessing_cfg.min_duration_off = trial.suggest_float("min_duration_off", cfg.optuna_min_duration_off_min, cfg.optuna_min_duration_off_max, step=step)
...
def diarization_objective(
    ...
    with tempfile.TemporaryDirectory(dir=temp_out_dir, prefix="Diar_PostProcessing_") as _:
        if trial is not None:
            postprocessing_cfg = optuna_suggest_params(postprocessing_cfg, trial, cfg)
...
def convert_pred_mat_to_segments(
    ...
    Uses predlist_to_timestamps_fast_pp (vectorized binarization + filtering) for both
    bypass and full postprocessing modes. This is ~100x faster than the original
    predlist_to_timestamps which uses a Python-level frame loop + repeat_interleave.
    ...
    total_speaker_timestamps = predlist_to_timestamps_fast_pp(
