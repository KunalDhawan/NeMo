...
python $BASEPATH/neural_diarizer/e2e_diarize_speech.py \
    model_path=/path/to/diar_nextformer.nemo.nemo \
    batch_size=1 \
...
class DiarizationConfig:
    ...
    # Eval Settings: (0.25, False) should be default setting for eval.
...
def main(cfg: DiarizationConfig) -> Union[DiarizationConfig]:
    ...
    if cfg.model_path.endswith(".ckpt"):
        diar_model = NextformerEncLabelModel.load_from_checkpoint(
            checkpoint_path=cfg.model_path, map_location=map_location, strict=False
        )
    elif cfg.model_path.endswith(".nemo"):
        diar_model = NextformerEncLabelModel.restore_from(restore_path=cfg.model_path, map_location=map_location)
    else:
