...
from nemo.utils import logging, config_utils
...
python transcribe_speech_spk.py \
    model_path=null \
...
def main(cfg: TranscriptionConfig) -> Union[TranscriptionConfig, List[Hypothesis]]:
    ...
    with autocast(dtype=amp_dtype):
        with torch.no_grad():
            if partial_audio:
                transcriptions, spk_mappings= transcribe_partial_audio(
                    asr_model=asr_model,
...
def write_transcription_to_seglst(
