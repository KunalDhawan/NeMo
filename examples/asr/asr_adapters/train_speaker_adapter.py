...
def main(cfg):
    ...
    if cfg.model.pretrained_asr_model is None and cfg.model.nemo_model is None:
        raise ValueError("Either set `cfg.model.nemo_model` or `cfg.model.pretrained_model`")
    if cfg.model.pretrained_asr_model is not None and cfg.model.nemo_model is not None:
    ...
    if cfg.model.pretrained_asr_model is not None:
        model_cfg = EncDecRNNTBPEQLTSASRModel.from_pretrained(cfg.model.pretrained_asr_model, return_config=True)
        update_model_config_to_support_adapter(model_cfg, cfg)
        model = EncDecRNNTBPEQLTSASRModel.from_pretrained(cfg.model.pretrained_asr_model, override_config_path=model_cfg, trainer=trainer)
    else:
        model_cfg = EncDecRNNTBPEQLTSASRModel.restore_from(cfg.model.nemo_model, return_config=True)
        update_model_config_to_support_adapter(model_cfg, cfg)
        model = EncDecRNNTBPEQLTSASRModel.restore_from(cfg.model.nemo_model, override_config_path=model_cfg, trainer=trainer)
    
    model._init_diar_model(
        model_path=cfg.model.diar_model_path,
        num_speakers=cfg.model.num_speakers,
        freeze_diar=cfg.model.freeze_diar,
    ...
    if cfg.model.using_adapter:
        encoder_adapter_cfg = LinearAdapterConfig(
            in_features=cfg.model.model_defaults.enc_hidden,
            dim=cfg.model.adapter_dim,
            bias=cfg.model.adapter_bias,
        )
        decoder_adapter_cfg = LinearAdapterConfig(
            in_features=cfg.model.model_defaults.enc_hidden,
            dim=32,
        )
        joint_adapter_cfg = LinearAdapterConfig(
            in_features=cfg.model.model_defaults.joint_hidden,
            dim=32,
        )
        model.add_adapter('encoder:encoder_adapter', cfg=encoder_adapter_cfg)
        model.add_adapter('decoder:decoder_adapter', cfg=decoder_adapter_cfg)
        model.add_adapter('joint:joint_adapter', cfg=joint_adapter_cfg)
        ...
        model.set_enabled_adapters('encoder:encoder_adapter', enabled=True)  # enable just one adapter by name
        model.set_enabled_adapters('decoder:decoder_adapter', enabled=True)  # enable just one adapter by name
        model.set_enabled_adapters('joint:joint_adapter', enabled=True)  # enable just one adapter by name
        ...
        model = model.train()
        model.diarization_model.freeze()
